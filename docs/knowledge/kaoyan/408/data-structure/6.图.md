---
title: '6.图'
description: '计算机 408 数据结构「6.图」知识笔记，涵盖1.逻辑结构、1.1 定义、1.2 【考点】术语、1.2.1有向图等内容，适合系统复习、章节回顾与查漏补缺。'
---

# 一、图Graph

![在这里插入图片描述](/kaoyan-408-data-structure-assets/2952a99ea733184d8cb2.png)
...
![6有向图](/kaoyan-408-data-structure-assets/431c42287d70955f3a21.png)
...
![6无向图](/kaoyan-408-data-structure-assets/569df6f17c759176947f.png)
...
![6简单图](/kaoyan-408-data-structure-assets/c6ddd571d7c0165d347b.png)
...
- <mark>**子图**</mark>：设有两个图 G=(V, E) 和 G'=(V', E')， 若 V' 是 V 的子集，且 E' 是 E 的子集，则称 G' 是 G 的**子图**。
- <mark>**生成子图**</mark>：若有满足 V(G')= V(G) 的子图 G'，则称其为 G 的**生成子图**。下图中 G3 为 G1 的子图。
...
- **<mark>连通图</mark>**：若图 G 中**任意两个顶点都是连通的**，则称图G为**连通图**，否则称为**非连通图**。

若一个图有n个顶点，并且边数小于n - 1，则此图必是非连通图。

- <mark>**连通分量**</mark>：无向图中的**极大连通子图**称为**连通分量**。如下图(a)所示，图G4有3个连通分量，如图(b)所示。

![6连通图.png](/kaoyan-408-data-structure-assets/dbee376dc2eb92d0d079.png)

**极大连通子图**是无向图的连通分量，**极大**即要求**该连通子图**<mark>包含其所有的**边**</mark>。

**极小连通子图**是既要保持图**连通**又要使得边数最少的子图。

##### <mark>【考点】形成一个连通图：顶点与边关系</mark>
...
>
> <img src="/kaoyan-408-data-structure-assets/52ffd444e9dac6c71228.png" alt="6连通图-例题" style="zoom: 50%;" /><img src="/kaoyan-408-data-structure-assets/8b47917c8c843cb73c47.png" alt="6连通图-例题2" style="zoom:50%;" />
>
...
##### <mark>【考点】求有向图强连通分量的个数</mark>

【巧解】求强连通分量个数

**<mark>【技巧总结】把每一个成环后的强连通分量看作一个整体</mark>**，然后重新画图，直到没有新的环出现，此时有几个顶点，就是有几个强连通分量。
...
![6图生成树](/kaoyan-408-data-structure-assets/c144e42275540e7137b9.png)
...
<mark>入度</mark>是以顶点**v为终点**（**进入v**）的有向边的数目，记为 **ID(v)**。

<mark>出度</mark>是以顶点**v为起点**（**从v出去**）的有向边的数目，记为 **OD(v)**。
...
- <mark>路径</mark>：<u>路径上**边的数目**称为**路径长度**</u>。

- <mark>回路</mark>：第一个顶点和最后一个顶点相同的路径称为**回路**或**环**。
...
- <mark>简单路径</mark>：在路径序列中，**顶点不重复出现**的路径称为简单路径。

  **最短路径**一定是简单路径。

- <mark>简单回路</mark>：除第一个顶点和最后一个顶点外，其余顶点不重复出现的回路称为简单回路。

- <mark>路径长度</mark>：一条路径上，边的数目。

- <mark>距离</mark>（点到点的距离）：从顶点u出发到顶点v的**最短路径**若存在，则最短路径的长度称为从u到v的距离。若从u到v根本不存在路径，则记该距离为无穷(∞)。
...
![6邻接矩阵](/kaoyan-408-data-structure-assets/402d2fc3a0df4c88ca10.png)
...
![在这里插入图片描述](/kaoyan-408-data-structure-assets/f4e3a81df7f9a3511764.png)
...
![在这里插入图片描述](/kaoyan-408-data-structure-assets/7e61b5fca2232d171df1.png)
...
![A[i][j]](/kaoyan-408-data-structure-assets/b419e88cffe25e35d141.png)

![img](/kaoyan-408-data-structure-assets/376da9d431f07034f299.png)
...
![邻接矩阵相乘](/kaoyan-408-data-structure-assets/aa0f23ffb864c96d13a1.png)

#### <mark>【考点】矩阵内元素的含义</mark>
...
![邻接矩阵相乘2](/kaoyan-408-data-structure-assets/67cbbd8a603785c3a853.png)
...
![在这里插入图片描述](/kaoyan-408-data-structure-assets/334366d2ff4e543606f1.png)
...
![邻接表](/kaoyan-408-data-structure-assets/d74b715ba4697b6c5dfe.png)

**链表的结点，没有先后顺序，都是直接连在顶点上的**。所以链表有很多表示方式，**不唯一**。

> 一个参考图：
>
> ![在这里插入图片描述](/kaoyan-408-data-structure-assets/6c02e0f73d3fcecabab6.png)
...
![在这里插入图片描述](/kaoyan-408-data-structure-assets/5fa06afb04b8358d5fd9.png)

### 2.2有向图

有向图存储中，边结点的数量是|E|，整体空间复杂度为 O(|V|+ |E|)。

![在这里插入图片描述](/kaoyan-408-data-structure-assets/e3fb8b4fb7759a699bac.png)
...
- 遇到邻接矩阵，先看是不是**<mark>对称矩阵</mark>**。
  - <mark>对称矩阵：无向图</mark>
  - <mark>非对称矩阵：有向图</mark>

- 遇到邻接表，先看**边表结点**的个数和**<mark>奇偶</mark>**。
  - <mark>奇数</mark>个边：一定是<mark>有向</mark>邻接表。
  - <mark>偶数</mark>个边：一定是<mark>无向</mark>邻接表。（因为在无向图邻接表，每个边存储两次）
...
3. 但是，若要确定给定的**两个顶点间边**是否存在，则在邻接矩阵中可以立刻查到，而在邻接表中则需要在相应结点对应的边表中查找另一结点，效率较低。

4. 在**无向图**的邻接表表示中，因为每一条边都存储两次，所以**边表结点**的个数一定是**偶数**。如果边结点个数为奇数，那么就是有向图。

   <mark>有 n 个顶点，最多边数是 $\C_n^2$，最多边表结点是 $2\C_n^2$.</mark>
...
![image-20240518081851132](/kaoyan-408-data-structure-assets/e0f82df467f91bd483c5.png)
...
>
> ![在这里插入图片描述](/kaoyan-408-data-structure-assets/febec44ae158fab7a9bd.png)
...
![image-20240518090717211](/kaoyan-408-data-structure-assets/8b16b10b61c2e849325c.png)

删除一条边，就改变它的两个link指针。

![image-20240518090906723](/kaoyan-408-data-structure-assets/7bec0e449320944ea59c.png)

删除一个顶点，那么就把对应的边表清空。

![image-20240518091051319](/kaoyan-408-data-structure-assets/516c501d492cb181e4f9.png)
...
![在这里插入图片描述](/kaoyan-408-data-structure-assets/92b42da7b55349b25d9f.png)
...
首先访问图中某一起始顶点v，然后由v出发，访问与v邻接且未被访问的任一顶点w1，再访问与 w1 邻接且未被访问的任一顶点…重复上述过程。当不能再继续向下访问时，依次**退回到<mark>最近</mark>被访问的顶点**，若它还有邻接顶点未被访问过，则从该点开始继续上述搜索过程，直至图中所有顶点均被访问过为止。

![6DFS树](/kaoyan-408-data-structure-assets/c18954794e2735e1c845.png)
...
![image-20240528092316722](/kaoyan-408-data-structure-assets/932ccff69e02dcead430.png)
...
![6DFS生成树](/kaoyan-408-data-structure-assets/4048b6f3785a41e66373.png)

**【注意】**当图里面有多个**连通分量**，那么就会有多个生成树，这时候这些树就组成一个生成森林。

![在这里插入图片描述](/kaoyan-408-data-structure-assets/9dc583c46ac435e5b3a4.png)
...
![6BFS](/kaoyan-408-data-structure-assets/cd429e1566f18aaec5da.png)
...
![6BFS广度优先生成树](/kaoyan-408-data-structure-assets/7bbabfd0f429e8d852f0.png)

【注意】BFS算法得到的遍历序列就是广度优先生成树。

- 因为**邻接矩阵**的表示方式是唯一的，所以BFS算法得到的遍历序列**是唯一**的。
- 因为**邻接表**的表示方式不是唯一的，所以BFS算法得到的遍历序列**不是唯一**的。

![6BFS生成树不唯一](/kaoyan-408-data-structure-assets/f2775c48b5373ddbf4fc.png)

当图里面有多个连通分量，那么就会有多个生成树，这时候这些树就组成一个生成森林。

### 1.3<mark>【考点】遍历的唯一性</mark>
...
## 2.<mark>【考点】最小生成树MST</mark>

一个图可以有多个生成树，我们定义无向连通图的 **最小生成树**（Minimum Spanning Tree，MST）为所有**边的权值的和最小**的生成树。

**【注意】**：

1. <mark>最小生成树也**可能有多个（不唯一）**。</mark>
   - <mark>虽然生成树不唯一，但是**代价**（所有边的权值的和）一定一样。</mark>
   - 【2017年408大题】<mark>只要无向连通图中的边权值**都不一样**，那么最小生成树**唯一**。</mark>
   - 同理，如果有权值一样的，那么生成树就不唯一。

2. <mark>一个连通图的生成树是一个**极小的连通子图**，它含有图中全部的顶点。</mark>
...
### 2.1 <mark>【考点】普里姆(Prim)算法</mark>

**【总结，记忆】<mark><u>从一个顶点开始</u>，每次将代价最小的新顶点纳入生成树,直到所有顶点都纳入为止。</mark>**

![prim](/kaoyan-408-data-structure-assets/0248c4ed6f9f750ff826.png)
...
> 然后进行第一轮low的遍历
>
> ![prim算法1](/kaoyan-408-data-structure-assets/9fd9be2f9796c8063aa5.png)
>
> 修改lowCost之后，修改isJoin：
>
> ![6prim算法2](/kaoyan-408-data-structure-assets/d3dd17a9ba516c7a17b4.png)
>
> ![image-20240605092409632](/kaoyan-408-data-structure-assets/5949605c649745fc1499.png)
>
> 然后这个点就结束了，再去寻找下一个lowCost最低的点：
>
> ![image-20240605092326006](/kaoyan-408-data-structure-assets/4c423895033d3766bd33.png)
>
> ...一直扫描所有点，直到isJoin全部为true：
>
> ![image-20240605092538186](/kaoyan-408-data-structure-assets/c2517fb1144e2bc1ba18.png)
...
### 2.2 <mark>【考点】克鲁斯卡尔(Kruskal)算法</mark>

与Prim算法从顶点开始扩展最小生成树不同，Kruskal 算法是一种按权值的递增次序选择合适的边来构造最小生成树的方法。

**【总结，记忆】<mark>这个算法不选择一开始的顶点，<u>直接找权值最小的边</u>，从小到大加入边，是个贪心算法。</mark>**

![Kruskal](/kaoyan-408-data-structure-assets/09aaa36fc10f1fe44c95.png)
...
> ![image-20240605092759512](/kaoyan-408-data-structure-assets/1c4e8ccb15b19714cabd.png)
...
![eg](/kaoyan-408-data-structure-assets/8c7aa96e1bef979c6a5f.apng)
...
### 3.2<mark>【考点】迪杰斯特拉(Dijkstra)算法</mark>
...
1. <mark>初始化：是把一个顶点放入并查集。</mark>
2. <mark>根据并查集内的点，找到能走**一步**到达的路径，并把这些走一步的路径里面的路径**权值最小**的那个到达的点放入并查集，并锁定这条路径（就是**最短路径**）。</mark>
3. <mark>结束：直到全部的点都在并查集结束。</mark>

**【注意】每一轮都会确定一个点的最短路径**，也就是说有 n 个点，就需要进行 n-1 轮。

那么时间复杂度就是 O(n^2^)。因为每轮会遍历一遍n个结点。

【五分钟速通】Dijkstra算法手动模拟流程 - Prism菌

![6Dijstra算法](/kaoyan-408-data-structure-assets/4ff1731d23387277e6fb.png)

实在是不懂看视频：6.4_3_最短路径问题_Dijkstra算法_哔哩哔哩_bilibili

![Dijstra算法](/kaoyan-408-data-structure-assets/da1df2e16d0756e03039.png)
...
![image-20240528213138084](/kaoyan-408-data-structure-assets/581b5c8bf402d0723362.png)

【例子】：

![6fuloyd例子1](/kaoyan-408-data-structure-assets/b8b4ffe813869f630a0c.png)
...
![6fuloyd例子2](/kaoyan-408-data-structure-assets/6437560d5503ffd7619c.png)

从这个表中，可以发下一些规律：

![6fuloyd例子3](/kaoyan-408-data-structure-assets/3a1fe40a059d24954873.png)

**【技巧】可以看出，矩阵中，每一步中红线划掉的部分都不用考虑计算，只需要计算红线外的部分，节省了计算量。**

#### <mark>【考点 - 弗洛伊德算法模拟】</mark>

【五分钟速通】巨快无比秒杀Floyd算法手动模拟流程 - Prism菌
...
![6fuloyd-手算](/kaoyan-408-data-structure-assets/26878f597b8d3a5e5a89.png)
...
![6DAG图](/kaoyan-408-data-structure-assets/bc6d2db7f8957479713e.png)

对于使用树来表示算术表达式，会可能有重复的部分：

![6DAG-树描述表达式](/kaoyan-408-data-structure-assets/84b959d83168f66dab0c.png)

这棵树中，红色和绿色的子树是重复的，那么就要可以删去一个：

![6DAG描述表达式](/kaoyan-408-data-structure-assets/8baebfaa1c0fd5bd7822.png)

同理，还可以合并多处重复的数据：

![6DAG描述表达式2](/kaoyan-408-data-structure-assets/8a2ddc02338899b7d5ca.png)
...
> 答案：A.5
>
> ![6DAG描述表达式-例题](/kaoyan-408-data-structure-assets/14b1919fbe9248f738f6.png)

#### 4.1.1<mark>【考点】简化表达式解题步骤</mark>

![6DAG描述表达式-解题步骤](/kaoyan-408-data-structure-assets/e23433c4f10770a8b20b.png)
...
![6AOV网例子](/kaoyan-408-data-structure-assets/cd60e452544322da1dc5.png)

#### 4.2.1 定义

所谓**拓扑排序，其实就是对一个有向图构造拓扑序列的过程**。每个AOV网都有一个或多个拓扑排序序列。

#### 4.2.2<mark>【考点】算法（求拓扑排序的序列）</mark>

拓扑排序的实现步骤：

1. <mark>从AOV网中选择一个没有前驱（**入度为0**）的顶点并输出。</mark>
2. <mark>从网中删除该顶点和所有以它为起点的有向边。</mark>
3. <mark>重复①和②直到当前的**AOV网为空**或当前**网中不存在无前驱的顶点**为止。</mark>

**【注意】**

1. **一定是<mark>无环图</mark>才有拓扑排序，有环则不行。**如果输出顶点数少了，哪怕是少了一个，也说明这个网存在环(回路)，不是AOV网。
   - 有向图**不能**排成拓扑序列：**顶点数大于1的强连通分量**
   - 【2012年408真题】有向图**能**排成拓扑序列：**邻接矩阵主对角线下面元素都是 0**

2. 若一个顶点有**多个直接后继**，**则拓扑排序的结果通常不唯一**。（当然要以具体情况为准）
3. 通过拓扑排序，**无法**获得唯一的图（拓扑 **—/→** 图）。

【技巧】为了不漏掉序列次数，可以把固定前后关系但画出来。

![eg](/kaoyan-408-data-structure-assets/450b7f71c34cfc1bdbd0.png)
...
![6AOE网](/kaoyan-408-data-structure-assets/b55458b2e855560e2da0.png)
...
- <mark>**关键路径**</mark>：从源点到汇点的有向路径可能有**多条**，所有路径中，具有<mark>**最大路径长度**</mark>的路径称为**关键路径**，而把关键路径上的活动称为**关键活动**。
...
2. 当网中的**关键路径并不唯一**，对于有几条关键路径的网，只提高一条关键路径上的关键活动速度并不能缩短整个工程的工期，只有加快那些**包括在所有关键路径上的<mark>共有的</mark>关键活动（边权值）**才能达到缩短工期的目的。
...
5. **【2022年408真题】时间余量**：一个活动a的最迟开始时间`l(i)`和其最早开始时间`e(i)`的**差额**`d(i) = l(i) - e(i)`：它是指该活动完成的**时间余量**，即在不增加完成整个工程所需总时间的情况下，活动a可以拖延的时间。

   若一个活动的**时间余量为零（不允许拖延）**，则说明该活动必须要如期完成，否则就会拖延整个工程的进度，所以称`l(i)-e(i)=0`即`l(i)= e(i)`的活动a是**关键活动**。

![6关键路径](/kaoyan-408-data-structure-assets/c24b23304b8749c74bc6.png)

### <mark>【考点】求关键路径</mark>

![6找关键路径](/kaoyan-408-data-structure-assets/e5056f070786d026671b.png)
...
- 点（事件）
  1. 从源点出发，令`ve(源点)=0`，按**拓扑排序**求其余顶点的最早发生时间`ve()`。
  2. 从汇点出发，令`vl(汇点)= ve(汇点)`，按**逆拓扑排序**求其余顶点的最迟发生时间`vl()`。
- 边（活动）
  1. 根据各顶点的ve()值求所有弧的最早开始时间e()。
     - `e()` = 出发的那个点的 `ve()`。
  2. 根据各顶点的vl()值求所有弧的最迟开始时间l()。
     - `l()` = 结束的那个点的 `vl() - 权值`。
- 求AOE网中所有活动的差额`d() = l - e`，找出所有`d()=0`的活动构成**关键路径**。

【技巧】这里的事件的值，其实就是第几天开始，值越大就是越晚，越小就是越早。

| 说明                                                                                                                                                                                                                                                       |      | 源点V1 | V2  | V3    | V4    | V5  | 汇点V6 |
| :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ------ | --- | ----- | ----- | --- | ------ |
| 正向（拓扑排序）推算事件(结点)<mark>最**早**</mark>发生的时间$Ve$<br />（如果有两个前驱得到两个值，<mark>**选大的**</mark>，因为早完成的需要等晚完成的）                                                                                                   | $Ve$ | 正向0  | 3   | 2     | 6     | 6   | 8      |
| 反向（逆拓扑排序）推算事件(结点)<mark>最**晚**</mark>发生的时间$Vl$<br />（如果有两个后继得到两个值，<mark>**选小的**</mark>，因为小的表示早完成的那个，需要早完成后继才能正常进行）<br />**【注意】源点、汇点是不变的**，就是上面求的最早发生时间的数字。 | $Vl$ | 反向0  | 4   | 2     | 6     | 7   | 8      |
| $Vd = Vl - Ve$<br />Vd为0，表示没有商量的余地，所以是**关键事件**。                                                                                                                                                                                        | $Vd$ | **0**  | 1   | **0** | **0** | 1   | **0**  |

<mark>【考点：求关键路径长度】</mark>如果是求关键路径长度，那么只需要正向拓扑排序算出汇点的值即可，不需要再求逆向的，找关键事件。

把vd=0的关键事件结点都带上：所以<mark>关键路径</mark>是：V1->V3->V4->V6。

| 说 明                            | . .       | <1,v2>                       | <v1,v3> | <v2,v4> | <v2,v5> | <v3,v4> | <v3,v6> | <v4,v6> | <v5,v6> |
| -------------------------------- | --------- | ---------------------------- | ------- | ------- | ------- | ------- | ------- | ------- | ------- |
| `e` = 出发的那个点的 `ve`        | e         | v1=0                         | 0       | v2=3    | 3       | v3=2    | 2       | v4=6    | v5=6    |
| `l` = 结束的那个点的 `vl - 权值` | l         | vl(v2)=4，权值=1 <br />4-1=3 | 2-2=0   | 6-2=4   | 4       | 2       | 5       | 6       | 7       |
| d=0为**关键活动**。              | d = l - e |                              | 0       |         |         | 0       |         | 0       |         |
